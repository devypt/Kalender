...
public class TableOfEvents 
	...
	private static final String COL_NAME = Trans.t("Name"), COL_DATE = Trans.t("Date"),
			COL_FREQ = Trans.t("Regelmigkeit"), COL_TIME = Trans.t("time of day"),
			COL_HOLIDAY = Trans.t("holiday"), COL_ID = "ID";
	
	/** default text shown in the search text field */
	private static final String DEFAULT_FILTER_TEXT = Trans.t("Name, date, or time");
	...
	/** control buttons */
	private JButton editButton, deleteButton, newButton, remindButton;
	...
	private void arrangeDialog() {
		...
		remindButton = new JButton(Trans.t("Reminder"));
		newButton = new JButton(Trans.t("New"));
		editButton.setToolTipText(Trans.t("Edit the selected event"));
		deleteButton.setToolTipText(Trans.t("Delete the selected event"));
		remindButton.setToolTipText(Trans.t("The alarm for this event set"));
		newButton.setToolTipText(Trans.t("A new event for this day to create"));
		...
		deleteButton.setIcon(new ImageIcon(this.getClass().getClassLoader().getResource("media/delete_event20.ico")));
		editButton.setIcon(new ImageIcon(this.getClass().getClassLoader().getResource("media/edit_event20.ico")));
		...
		if (date == null) {
			if ((holidays.size() > 0 && holidaysOnly) ||
				...
				JLabel filterLabel = new JLabel(Trans.t("search")+": ");
			...
			eventPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), 
					date != null ? Trans.t("events") : Trans.t("overview"), TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, 
			...
			if (caller.getConfig().getMoon() && 
				...
				if (moon == Moon.MOON_DEC_HALF) {
					moonImage = new JLabel(new ImageIcon(this.getClass().getClassLoader().getResource("media/half-desc.png")));
					moonText = Trans.t("Waning Crescent");
				}
				else if (moon == Moon.MOON_FULL) {
					moonImage = new JLabel(new ImageIcon(this.getClass().getClassLoader().getResource("media/full.png")));
					moonText = Trans.t("full moon");
				}
				else if (moon == Moon.MOON_INC_HALF) {
					moonImage = new JLabel(new ImageIcon(this.getClass().getClassLoader().getResource("media/half-asc.png")));
					moonText = Trans.t("Waxing Crescent");
				}
				else if (moon == Moon.MOON_NEW) {
					moonImage = new JLabel(new ImageIcon(this.getClass().getClassLoader().getResource("media/new.png")));
					moonText = Trans.t("new moon");
				}
				moonText += " "+Trans.t("on the night of")+" " + date.dateToString(false) + ".";
				...
				moonPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), 
						Trans.t("moon Phase"), TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, 
						Const.FONT_BORDER_TEXT));
			...
			if (holidays.size() > 0) {
				...
				holidayPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), 
						Trans.t("Holidays"), TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, 
						Const.FONT_BORDER_TEXT));
		...
		if (events.size() == 0) {
			...
			noEvents.add(new JLabel(holidaysOnly ? Trans.t("no holidays registered") :
					Trans.t("no scheduled events")+"..."));
...
//			eventTable.setAutoCreateRowSorter(true);
	 ...
	 */
	public TableOfEvents(Date d, Calendar c, boolean holidaysOnly) {
		...
		String title;
		if (d == null)
			title = "Alle " + (holidaysOnly ? Trans.t("legal and other Holidays") : 
				Trans.t("events")) + " (" + events.size() + ")";
		else {
			long dayDiff = d.dayDiff(new Date());
			if (dayDiff == 0)
				title = Trans.t("today")+" (" + d.dateToString(false) + ")";
			else if (dayDiff == 1)
				title = Trans.t("morning")+" (" + d.dateToString(false) + ")";
			else if (dayDiff == 2)
				title = Trans.t("Tomorrow")+" (" + d.dateToString(false) + ")";
			else if (dayDiff == -1)
				title = Trans.t("yesterday")+" (" + d.dateToString(false) + ")";
			else
	...
	private void filter() {
		...
		if (text.equals("")) {
			...
			setTitle(Trans.t("all")+" " + (holidaysOnly ? Trans.t("legal and other Holidays") : 
					Trans.t("events")) + " (" + events.size() + ")");
		}
		 ...
		 */
			...
			setTitle((holidaysOnly ? Trans.t("Holidays") : Trans.t("events")) + " "+Trans.t("with")+" \"" +
	...
	public void actionPerformed(ActionEvent a) {
		...
		if (a.getSource().equals(editButton))
			new EditEvent(caller, selectedEvent);
