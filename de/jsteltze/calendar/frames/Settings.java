...
public class Settings 
	...
	private JPanel arrangeTab1() {
		...
		autoUpdateBox = new JCheckBox(Trans.t("Automatically check for updates"));
		autoUpdateBox.setSelected(caller.getConfig().getAutoUpdate());
		
		moonBox = new JCheckBox(Trans.t("Moon Show"));
		moonBox.setSelected(caller.getConfig().getMoon());
		
		systrayBox = new JCheckBox(Trans.t("Start in the systray"));
		systrayBox.setSelected(caller.getConfig().getSystrayStart());
		
		playThemeBox = new JCheckBox(Trans.t("Music playback")+":");
		playThemeBox.setSelected(caller.getConfig().getPlayTheme());
		playThemeBox.addItemListener(this);
				
		ownThemeBox = new JCheckBox(Trans.t("own"));
		...
		playThemeButton.setToolTipText(Trans.t("playing a melody"));
		...
		p11.add(new JLabel(" "+Trans.t("Standard distance")+": "));
		p11.add(remindBox);
		...
		p21.add(moonBox);
		p22.add(new JLabel(Trans.t("Look and Feel")));
		p22.add(styleBox);
		p2.add(p21);
		p2.add(p22);
		...
		p31.add(new JLabel(" "+Trans.t("Behavior when the program")+": "));
		p31.add(onCloseBox);		
		p32.add(autoUpdateBox);
		p34.add(systrayBox);
		...
		p33.add(new JLabel(" "+Trans.t("working Directory")+": "));
		p33.add(workingDirLabel);
	...
	private JPanel arrangeTab2() {
		...
		holidayBoxes[1] = new JCheckBox("Heilige 3 K�nige (6.1.)");
		holidayBoxes[2] = new JCheckBox("Gr�ndonnerstag (variabel)");
		...
		holidayBoxes[9] = new JCheckBox("Mari� Himmelfahrt (15.8.)");
		...
		holidayBoxes[13] = new JCheckBox("Bu�- und Bettag (variabel)");
	...
	private JPanel arrangeTab3() {
		...
		JPanel pS = new JPanel(new FlowLayout(FlowLayout.RIGHT));
		VerticalFlowPanel pRadiosAll = new VerticalFlowPanel();

		ColorChooser Palette = new ColorChooser(this);
		Palette.setBounds(20, 20, 128, 152);
		JLabel greenL = new JLabel(Trans.t("blue"));
		greenL.setUI(new VerticalLabelUI(false));
		greenL.setBounds(2, 20, 20, 128);
		greenL.setHorizontalAlignment(SwingConstants.RIGHT);
		JLabel blueL = new JLabel(Trans.t("Green"));
		blueL.setBounds(20, 2, 128, 20);
		JLabel redL = new JLabel(Trans.t("Red"));
		redL.setBounds(2, 144, 20, 20);

		pColorChooser.add(greenL);
		pColorChooser.add(blueL);
		pColorChooser.add(redL);
		pColorChooser.add(Palette);
		...
		for (byte i = 0x00; i < ColorSet.MAXCOLORS; i++) {
			...
			colorRects[i] = new JLabel(Trans.t("lalabla"));
...
//		pRadiosAll.setBorder(new EtchedBorder());
	...
	private JPanel arrangeTab4() {
		...
		titleLabel = new JLabel(Trans.t("Own settings and events")+":");
	...
	private JPanel arrangeTab5() {
		...
		updateButton = new JButton(Trans.t("Check for new version"));
		updateButton.addActionListener(this);

		JTextArea jt = new JTextArea(
				Calendar.FILENAME
				+"\nVersion " + Calendar.VERSION
				+"\nCompiler is " + Calendar.LAST_EDIT_DATE + " mit " + Calendar.COMPILER);
		jt.setBorder(new EtchedBorder());
		jt.setEditable(false);
		jt.setBackground(Const.COLOR_SETTINGS_INFO_BG);
		
		JLabel author = new JLabel(Calendar.AUTHOR + " (C) 2012");
		...
		pWest.add(new LinkLabel(Calendar.HOME_URL, Calendar.HOME_URL), BorderLayout.SOUTH);
	...
	private void arrangeFrame(int tabNo) {
		...
		tab = new JTabbedPane();
		tab.addTab(Trans.t("general"), arrangeTab1());
		tab.addTab(Trans.t("Holidays"), arrangeTab2());
		tab.addTab(Trans.t("colors"), arrangeTab3());
		tab.addTab(Trans.t("Import/Export"), arrangeTab4());
		tab.addTab(Trans.t("Program Info"), arrangeTab5());
	...
	public Settings(Calendar c, int tabNo) {
		super(c.getGUI().getFrame(), Trans.t("settings")+"...");
		caller = c;
		setIcon();
		arrangeFrame(tabNo);
	}
	...
	private JPanel fillTable(Vector<Event> e, Configuration c) {
		...
		if (!c.equals(Configuration.defaultConfig)) {
			if (c.getView() != Configuration.defaultConfig.getView())
				configLines.add(Trans.t("Standard View")+": " + Configuration.VIEW_LABELS[c.getView()]);
			...
			if (c.getOnCloseAction() != Configuration.defaultConfig.getOnCloseAction())
				configLines.add(Trans.t("when closing")+": " + Configuration.ON_CLOSE_LABELS[c.getOnCloseAction()]);
			...
			if (c.getAutoUpdate() != Configuration.defaultConfig.getAutoUpdate())
				configLines.add(Trans.t("Automatically check for updates")+": " + 
						(c.getAutoUpdate() ? Trans.t("Yes") : Trans.t("No")));
			if (c.getMoon() != Configuration.defaultConfig.getMoon())
				configLines.add(Trans.t("Moon Show")+": " + 
						(c.getMoon() ? Trans.t("Yes") : Trans.t("No")));
			if (c.getHolidays() != Configuration.defaultConfig.getHolidays())
				configLines.add(Trans.t("Holidays"));
			if (c.getSpecialDays() != Configuration.defaultConfig.getSpecialDays())
				configLines.add(Trans.t("special Days"));
			for (byte i = 0x00; i < ColorSet.MAXCOLORS; i++)
				if (!c.getColors()[i].equals(ColorSet.DEFAULT[i]))
					configLines.add(Trans.t("color")+" (" + ColorSet.NAMES[i] + ")");
		}

		for (Event ev : e)
			eventLines.add(ev.getDate().dateToString(true)
					+ (ev.getDate().hasTime() ? ", " + ev.getDate().timeToString() + " "+Trans.t("clock")+" " : " ")
		...
		if (configLines.size() > 0) {
			allImportExportBoxes[0] = new JCheckBox(Trans.t("All settings")+"...");
		...
		if (eventLines.size() > 0) {
			allImportExportBoxes[1] = new JCheckBox(Trans.t("All events")+"...");
		...
		if (numConfig + numEvents == 0)
			pScrollEvents.add(new JLabel(Trans.t("No data available for exporting")+"..."));
	...
	private void doImport() {
		...
		if (possibleConfig != Configuration.defaultConfig) {
			...
			caller.setConfig(new Configuration(view, reminder, atClose,
					style, colors, holidayID, specialID, autoUpdate, moon,
					theme, systrayStart, playTheme));
		}
		...
		for (int i = index; i < possibleEvents.size() + index; i++)
			if (importExportBoxes[i].isSelected())
				caller.newEvent(possibleEvents.elementAt(i - index), true);
	}
			...
			if (filename.endsWith(".xml")) {
				...
				try {
					...
					JOptionPane.showMessageDialog(this,
							Trans.t("The contents of the file")+" \"" + filename
							+ "\" "+Trans.t("does not fit into the scheme")+".\n> " + e.getMessage() + "\n"+Trans.t("The import can not be performed")+".",
							Trans.t("Error reading")+"...", JOptionPane.ERROR_MESSAGE);
					return;
	...
	private void doExport() {
		...
		if (!somethingSelected) {
			JOptionPane.showMessageDialog(this,
					Trans.t("There are no data to export selected")+".",
					Trans.t("export")+"...", JOptionPane.PLAIN_MESSAGE);
			return;
		}
		...
		if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
			...
			JOptionPane.showMessageDialog(this,
					Trans.t("The selected data in the file")+"\n" + filename
					+ "\n"+"saved" +".\n\n"+"Note: Anhnge and notes of events \nknnen not be exported"+".", "Export erfolgreich...",
	...
	public void actionPerformed(ActionEvent a) {
		 ...
		 */
			...
			try {
				...
				JOptionPane.showMessageDialog(caller.getGUI().getFrame(), Trans.t("The file can not be played. \n Error")+": "+ex.toString(), "Fehler beim Abspielen", JOptionPane.ERROR_MESSAGE);
			}
			return;
		}
	...
	public void itemStateChanged(ItemEvent i) {
		if (tab.getSelectedIndex() == TAB_COLORS) {
			...
			if (selectedColor == -1)
				return;
			colorField[0].setText("" + colorRects[selectedColor].getBackground().getRed());
			colorField[1].setText("" + colorRects[selectedColor].getBackground().getGreen());
			colorField[2].setText("" + colorRects[selectedColor].getBackground().getBlue());
		}
		else if (tab.getSelectedIndex() == TAB_IMEXPORT) {
			if (i.getSource().equals(importButton)) {
				scrollPane.setViewportView(new JPanel());
				importExportLabel.setText("<html><body>\"Datei...\" klicken,<br>um Datei zum<br>Importieren zu<br>w�hlen.</body></html>");
				titleLabel.setText("Noch keine Datei gew�hlt...");
	...
	public void keyReleased(KeyEvent k) {
		...
		colorRects[getSelectedColorIndex()].setForeground(new Color(r, g, b));
		colorRects[getSelectedColorIndex()].setBackground(new Color(r, g, b));
	}
