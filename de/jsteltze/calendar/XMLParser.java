...
public class XMLParser {
	...
	public void parse(String file) throws 
		...
		try {
			in = new Scanner(datei, de.jsteltze.calendar.Calendar.ENCODING);
		} catch (NoClassDefFoundError e) {
			JOptionPane.showMessageDialog(null,
					Trans.t("For execution is saturated with Java version 1.6 or later ben"),
					Trans.t("Java too old")+"...", JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		}
		...
		if (in.hasNextLine()) {
			...
			if (!zeile.substring(30).startsWith(de.jsteltze.calendar.Calendar.ENCODING))
				throw new CannotParseException("unknown XML-Encoding.");
		}

		if (in.hasNextLine()) {
			zeile = in.nextLine();
			if (!zeile.matches("<Calendar version=\"\\d\\.\\d_svn\\d+\">"))
				throw new CannotParseException("Line version is not as expected.");
		}
	...
	private void readConfigLines(Vector<String> configLines) {
		...
		this.config = new Configuration(defaultView, reminder, atClose,
				style, colors, holidays, specialDays, autoUpdate, moon,
				themeFile, startSystray, playTheme);
	}
