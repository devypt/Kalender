...
public class Cell {
	...
	private Color weekendColor, holidayBgColor, 
			holidayFtColor, ftColor, todayColor, selectedColor;
	 ...
	 * 		null.
	 */
	public Event getEvent(int y_coord, int height) {
		if (canvas.getView() == Configuration.VIEW_MONTH) {
			y_coord -= (M_CLEAR_UP + 27 + row * height);
			if (y_coord < 0)
				return null;
			...
			y_coord -= (W_CLEAR_UP + W_HEADER + 3 + row * height);
			if (y_coord < 0)
				return null;
			int index = Math.div(y_coord, fontsize);
	...
	/**
	 * Set today flag.
	 * @param x - True if the cells date is today
	 */
	public void setToday(boolean x) {
		today = x;
	}
	...
	private void printEventNameMonthWeek(Event event, Graphics x, Dimension space, int index) {
		...
		if (cuttedName.equals(event.getName())) {
			int neededSpace = x.getFontMetrics().stringWidth(cuttedName);
		...
		if (event.isSpecial() || event.isHoliday()) {
			...
			x.drawImage(BALLOONS_9, ++startText, start_y + header + 3 + index * fontsize, 
					event.isSelected() ? ColorUtil.addSaturation(selectedColor, _SATURATION) :
						Color.white, canvas);
			startText += 9;
		}
		...
		if (event.getEndDate() != null) {
			if (event.getDate().sameDateAs(this.date)) {
				...
				if (event.isSelected()) {
					...
					x.setColor(ColorUtil.addSaturation(x.getColor(), _SATURATION));
	...
	private void paintDay(Graphics x, Dimension space) {
		...
		x.fillRect(D_CLEAR_LEFT + 1 + col * space.width, D_CLEAR_UP + 1
				+ row * space.height, space.width - 1, space.height / 2);
		x.setColor(c2);
		x.fillRect(D_CLEAR_LEFT + 1 + col * space.width, D_CLEAR_UP + 1
				+ row * space.height + space.height / 2, space.width - 1, (space.height - 1) / 2);
