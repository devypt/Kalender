...
public class CalendarPanel 
	...
	private void arrangePanel() {
		...
		if (view == Configuration.VIEW_YEAR)
			toToday = new JButton(Trans.t("the current year"));
		else if (view == Configuration.VIEW_MONTH)
			toToday = new JButton(Trans.t("the current month"));
		else if (view == Configuration.VIEW_WEEK)
			toToday = new JButton(Trans.t("the current week"));
		else
			toToday = new JButton(Trans.t("the current day"));
		toNext.addActionListener(this);
		toPrev.addActionListener(this);
		toToday.addActionListener(this);
		jump.addButtonListener(this);
		toNext.setToolTipText(Trans.t("next"));
		toPrev.setToolTipText(Trans.t("previous"));
		jump.setToolTipText(Trans.t("jump to a specific date"));
		...
		leftBorder = new LeftPanelBorder(Toolkit.getDefaultToolkit().createImage(
				this.getClass().getClassLoader().getResource("media/leftpanelborder.PNG")),
				calendar.getConfig().getColors()[ColorSet.CONTROLPANEL]);
		leftBorder.setSize(11, 36);
		...
		rightBorder = new RightPanelBorder(Toolkit.getDefaultToolkit().createImage(
				this.getClass().getClassLoader().getResource("media/rightpanelborder.PNG")),
				calendar.getConfig().getColors()[ColorSet.CONTROLPANEL]);
		rightBorder.setSize(49, 37);
		...
		settings.setToolTipText(Trans.t("setting"));
		...
		eventsLabel = new LinkLabel((calendar.getAllEvents().size() - 
				...
				Holidays.getNumberOfHolidays(calendar.getConfig().getSpecialDays())) + " "+Trans.t("Events"),
				Trans.t("See all events"), (Window) null);
		eventsLabel.addMouseListener(this);
		holidaysLabel = new LinkLabel((Holidays.getNumberOfHolidays(calendar.getConfig().getHolidays()) +
				Holidays.getNumberOfHolidays(calendar.getConfig().getSpecialDays())) + " "+Trans.t("Holidays"),
				Trans.t("See all Holidays"), (Window) null);
		holidaysLabel.addMouseListener(this);
		notificationsLabel = new JLabel(calendar.getAlarmTasks().size() + " "+Trans.t("Active Holidays"));
	...
	public void updateStatusBar() {
		...
		eventsLabel.setText(num_events + " "+Trans.t("event") + (num_events == 1 ? "" : "se"));
		holidaysLabel.setText(num_holidays + " "+Trans.t("holiday") + (num_holidays == 1 ? "" : "e"));
		notificationsLabel.setText(num_notis + " "+Trans.t("memory") + (num_notis == 1 ? "" : "en") + " "+Trans.t("active"));
	}
	...
	public void setUI(byte style) {
		...
		try {
			...
			Logger.error("cannot set UI: " + e.toString());
		}
	...
	public void updateAvailable() {
		...
		infoLabel.setText(" >> "+Trans.t("It is a"));
		infoLabel.setForeground(Color.black);
		updateLabel = new LinkLabel("Update");
		updateLabel.addMouseListener(this);
		JLabel updateLabel2 = new JLabel(" "+Trans.t("available"));
	...
	public void mousePressed(MouseEvent m) {
		if (m.getSource().equals(settings)) {
			if (appletMode)
				putInfoMessage(Trans.t("IN DIESEM ONLINE-APPLET K�NNEN KEINE EINSTELLUNGEN GE�NDERT WERDEN"));
			else
	...
	public void mouseClicked(MouseEvent m) {
		...
		if (m.getSource().equals(eventsLabel))
			new TableOfEvents(null, calendar, false);
		
		else if (m.getSource().equals(holidaysLabel))
			new TableOfEvents(null, calendar, true);
	...
	private void jumpTo(Date x) {
		Logger.log("jump to date "+x.dateToString(false));
		...
		if (x.get(java.util.Calendar.YEAR) <= 1582) {
			JOptionPane.showMessageDialog(this,
					Trans.t("The Gregorian calendar is now used throughout the world"),
					Trans.t("Calendar border"), JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	public void buttonPressed(ImageButton x) {
		...
		if (x.equals(views[0]) || x.equals(views[1]) || x.equals(views[2])
			...
			if (views[0].isPressed()) {
				...
				toToday.setText(Trans.t("to current year"));
			} else if (views[1].isPressed()) {
				view = Configuration.VIEW_MONTH;
				toToday.setText(Trans.t("for the current month"));
			} else if (views[2].isPressed()) {
				view = Configuration.VIEW_WEEK;
				toToday.setText(Trans.t("to This Week"));
			} else if (views[3].isPressed()) {
				view = Configuration.VIEW_DAY;
				toToday.setText(Trans.t("the current day"));
			}
		 ...
		 */
			...
			if (view == Configuration.VIEW_YEAR) {
				...
				try {
					...
					JOptionPane.showMessageDialog(this,
							Trans.t("Invalid year"),
							Trans.t("errors"), JOptionPane.ERROR_MESSAGE);
				}
			} 
			else if (view == Configuration.VIEW_MONTH) {
				/*
				 ...
				 */
				String input = JOptionPane.showInputDialog(this,
						Trans.t("Month / year")+"(MM/JJJJ):", Trans.t("jump to")+"...",
						JOptionPane.PLAIN_MESSAGE);
				if (input == null)
					return;
				 ...
				 */
				String input = JOptionPane.showInputDialog(this,
						Trans.t("date")+" (TT.MM.JJJJ) "+Trans.t("or Week / year")+" (KW/JJJJ):", Trans.t("jump to")+"...",
						JOptionPane.PLAIN_MESSAGE);
				if (input == null)
					return;
				 ...
				 */
				String input = JOptionPane.showInputDialog(this,
						"Day / month / year (TT.MM.JJJJ):", Trans.t("jump to")+"...",
						JOptionPane.PLAIN_MESSAGE);
				if (input == null)
					return;
				int day, month, year;
